...
AgentCore CLIはTypeScript、CodeZip/Container、ローカル開発、検証、CDKベースのplan/deployを公式に扱う。
Mastra公式のAgentCore統合はBYO TypeScript agentをContainerでdeployする。
...
- Mastra Runtimeは `type: byo`、`build: Container` とし、CodeBuildでarm64 imageを作成する。
...
- 公式CLIの更新とMastra公式のContainer packagingを利用できる。
- デプロイスタックは複数になるため、出力値とデプロイ順序を明示する必要がある。
- CLIで不足する要件が出た場合はADRを更新して所有権を移す。

## Alternatives

- 全資源を独自CDKで管理: 一元化できるが、CLIのdev、validate、Container build/deployと重複するため不採用。
- Mastra RuntimeをCodeZipにする案: AgentCore CLIでは可能だが、Mastra公式手順がContainerを指定するため不採用。
